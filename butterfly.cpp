            ...
            for(double j = -major_radius; j <= major_radius; j += 1)
            {
                for(double k = -major_radius; k <= 0; k += 1)
                {
                       ...
                       (temp*temp == 4*major_radius*major_radius*(i*i + j*j) && mode == -1))
                    {
                        ...
                        //     for(int l = 0; l < 3; l++)
                        //     {
                        //         set_surface(x + 1.2*major_radius*(l-1), y + 1.2*major_radius*(w-1), z, gradient[int(N * ((k + major_radius) / (major_radius)))]);
                        //     }
                        // }
                        //butterfly
                        set_surface(x, y, z, gradient[int(N * ((k + major_radius) / (major_radius)))]);
                        set_surface(x + major_radius*2, y, z, gradient[int(N * ((k + major_radius) / (major_radius)))]);
                        set_surface(x, y + major_radius*2, z, gradient[int(N * ((k + major_radius) / (major_radius)))]);
                        set_surface(x + major_radius*2, y + major_radius*2, z, gradient[int(N * ((k + major_radius) / (major_radius)))]);

                        //complicated butterfly
                        set_surface(x, y, z + major_radius*2, gradient[int(N * ((k + major_radius) / (major_radius)))]);
                        set_surface(x + major_radius*2, y, z + major_radius*2, gradient[int(N * ((k + major_radius) / (major_radius)))]);
                        set_surface(x, y + major_radius*2, z + major_radius*2, gradient[int(N * ((k + major_radius) / (major_radius)))]);
                        set_surface(x + major_radius*2, y + major_radius*2, z + major_radius*2, gradient[int(N * ((k + major_radius) / (major_radius)))]);
                    }
                }
            }
        }
